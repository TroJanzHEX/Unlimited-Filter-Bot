...
async def get_all(client, message):
     ...
     b = "\n".join(texts)
     await message.reply_text(f'''Total number of filters in {title} {count}\n{b}''')
...
async def recive_filter(client,message):
       ...
       if fileid == "None":
          if btn == "[]":
              ...
              button = eval(btn)
              await message.reply_text(reply_text, parse_mode="html",reply_markup =  InlineKeyboardMarkup(button))

       else:
             if btn == "[]":
                await message.reply_cached_media(fileid, caption=reply_text or "")
             else:
                  button = eval(btn) 
                  await message.reply_cached_media(fileid,caption=reply_text or "",reply_markup =  InlineKeyboardMarkup(button))
